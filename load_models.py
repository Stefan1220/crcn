...
def create_crcn_blstm():
    model = Sequential()
    model.add(BLSTM(300, 300, return_sequences=True,init='he_normal',is_entity=True, regularize=True))
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
    model.add(Embedding(300, 512,init='he_normal'))
    model.add(Dropout(0.5))
...
def create_rcn_blstm():
    model = Sequential()
    model.add(BLSTM(300, 300, init='he_normal',return_sequences=True,is_entity=False, regularize=True))
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
    model.add(Embedding(300, 512,init='he_normal'))
    model.add(Dropout(0.5))
