...
json_imgs=jsondata['images']   #3000


features_path = os.path.join('./data/', 'example_travel.mat')     #3000*4096
features_struct = scipy.io.loadmat(features_path)['result']#.transpose()
...
for i,json_img in enumerate(json_imgs):
    ...
    if contents.has_key(pageurl):
        ...
        contents[pageurl].append({'imgid':str(i),'feature':imgfeature,'raw':concatstring,'tree':concattree})
    else:
        contents[pageurl]=[]
        #contents[pageurl].append({'imgid':str(i),'filename':json_img['filename'],'feature':imgfeature,'raw':concatstring,'tree':concattree})
        contents[pageurl].append({'imgid':str(i),'feature':imgfeature,'raw':concatstring,'tree':concattree})
...
Sentenceseq=np.zeros((training_num,MAX_SEQ_LEN,300)) #MAX_SEQ_LEN+1
Imageseq=np.zeros((training_num,MAX_SEQ_LEN,4096))  #4096
...
for i in range(1,20):
    ...
    model.fit(Sentenceseq, Imageseq, batch_size=100, nb_epoch=5,validation_split=0.1,shuffle=True,show_accuracy=True)  #batch_size=100
    print "Checkpoint saved"
    model.save_weights('./model/crcn_travel_'+str(i)+'.hdf5')
